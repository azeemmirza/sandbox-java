...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        ordering1(intList);
        List<String> stringList = Arrays.asList("cba", "efg", "doe", "fie", "set", "eoe");
    ...
    public static void ordering1(List<Integer> list) {
        List<Integer> result = list.stream()
                .sorted(Comparator.comparingInt((Integer x) -> Math.abs(x))
                        .thenComparingInt(x -> x)) // ensures -1 comes before 1, etc.
